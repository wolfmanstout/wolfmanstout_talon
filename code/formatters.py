...
def words_with_joiner(joiner):
    ...
    return (NOSEP, lambda i, word, _: ("" if i == 0 else joiner) + word.lower())
...
formatters_dict = {
    ...
    "DOT_SNAKE": (
        NOSEP,
        lambda i, word, _: "." + word.lower() if i == 0 else "_" + word.lower(),
    ),
    "SLASH_SEPARATED": (NOSEP, every_word(lambda w: "/" + w)),
    "CAPITALIZE_FIRST_WORD": (
        SEP,
        first_vs_rest(lambda w: w[0].capitalize() + w[1:] if w else ""),
    ),
...
formatters_words = {
    ...
    "studley": formatters_dict["PUBLIC_CAMEL_CASE"],
