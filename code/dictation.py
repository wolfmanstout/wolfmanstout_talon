...
@mod.capture(rule="(numb | num) <user.number_string> (dot | point) <digit_string>")
def prose_number_with_dot(m) -> str:
    return m.number_string + "." + m.digit_string

@mod.capture(rule="(numb | num) <user.number_string> colon <user.number_string>")
...
def auto_capitalize(text, state = None):
    ...
    for c in text:
        ...
        output += c
        newline = c == "\n"
        sentence_end = c in ".!?" and not no_cap_after.search(output + c)
